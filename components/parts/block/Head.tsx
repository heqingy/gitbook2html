...
export const RenderHead: React.FC<{ type: HeadType, data?: BlockData }> = ({ type, children, data }) => {
    switch (type) {
        case "heading-1":
            return <h1 style={{ fontSize: "24px", color: "#242A31", fontWeight: 700, lineHeight: 1.5, padding: "24px 0", borderTop: data?.isFirstEle ? 'none' : "1px solid #E6ECF1", marginTop: "24px" }}>
                {children}
            </h1>
        case "heading-2":
